...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        start_cursor = options.get("start_cursor") or saved_cursor or config.initial_cursor
        ...
        try:
            ...
            source_cursor.execute(source_query, preview_params)
            ...
            if not common_columns:
                raise CommandError(f"No common columns found between source table {config.source_table} and target table {config.target_table}.")
            ...
            with transaction.atomic():
                ...
                _mark_finished(config, latest_cursor, len(rows))

            self.stdout.write(self.style.SUCCESS(f"Synced {len(rows)} rows into {config.target_table}. Latest cursor: {latest_cursor}"))
